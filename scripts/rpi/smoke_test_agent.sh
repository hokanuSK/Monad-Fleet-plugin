...
if [[ -n "${SSH_STRICT_HOST_KEY_CHECKING}" ]]; then
  ...
  FALLBACK_SSH_ARGS+=(-o "StrictHostKeyChecking=${SSH_STRICT_HOST_KEY_CHECKING}")
fi
if [[ -n "${SSH_USER_KNOWN_HOSTS_FILE}" ]]; then
  SSH_ARGS+=(-o "UserKnownHostsFile=${SSH_USER_KNOWN_HOSTS_FILE}")
  FALLBACK_SSH_ARGS+=(-o "UserKnownHostsFile=${SSH_USER_KNOWN_HOSTS_FILE}")
fi
...
if [[ -n "${SSH_PROXY_JUMP}" ]]; then
  ...
  FALLBACK_SSH_ARGS+=(-J "${SSH_PROXY_JUMP}")
fi

...
SMOKE_PROFILE="${SMOKE_PROFILE:-full}"     # full|rssi|wireless_spec|rfpaper|design-json
...
OPS_OUTPUT_DIR="${OPS_OUTPUT_DIR:-${REPO_ROOT}/artifacts/output/ops}"
...
upload_window_from = (os.environ.get('UPLOAD_WINDOW_FROM') or '').strip()
upload_window_to = (os.environ.get('UPLOAD_WINDOW_TO') or '').strip()
upload_window_required = (os.environ.get('UPLOAD_WINDOW_REQUIRED') or '').strip()
upload_slot_count = (os.environ.get('UPLOAD_SLOT_COUNT') or '').strip()
upload_slot_jitter_s = (os.environ.get('UPLOAD_SLOT_JITTER_S') or '').strip()
prom_remote_write_on_cmd = (os.environ.get('PROM_REMOTE_WRITE_ON_CMD') or '').strip()
prom_remote_write_off_cmd = (os.environ.get('PROM_REMOTE_WRITE_OFF_CMD') or '').strip()
prom_remote_write_cmd_timeout_s = (os.environ.get('PROM_REMOTE_WRITE_CMD_TIMEOUT_S') or '').strip()
...
def with_upload_window_env(env_map: dict[str, str]) -> dict[str, str]:
    merged = dict(env_map)
    if upload_window_from:
        merged['UPLOAD_WINDOW_FROM'] = upload_window_from
    if upload_window_to:
        merged['UPLOAD_WINDOW_TO'] = upload_window_to
    if upload_window_required:
        merged['UPLOAD_WINDOW_REQUIRED'] = upload_window_required
    if upload_slot_count:
        merged['UPLOAD_SLOT_COUNT'] = upload_slot_count
    if upload_slot_jitter_s:
        merged['UPLOAD_SLOT_JITTER_S'] = upload_slot_jitter_s
    if prom_remote_write_on_cmd:
        merged['PROM_REMOTE_WRITE_ON_CMD'] = prom_remote_write_on_cmd
    if prom_remote_write_off_cmd:
        merged['PROM_REMOTE_WRITE_OFF_CMD'] = prom_remote_write_off_cmd
    if prom_remote_write_cmd_timeout_s:
        merged['PROM_REMOTE_WRITE_CMD_TIMEOUT_S'] = prom_remote_write_cmd_timeout_s
    return merged

def with_runtime_env(env_map: dict[str, str]) -> dict[str, str]:
    return with_upload_window_env(with_sink_env(with_artifact_upload_env(env_map)))
...
if upload_window_from or upload_window_to:
    reporting['upload_window'] = {
        'from': upload_window_from or policy['range']['from'],
        'to': upload_window_to or policy['range']['to'],
    }
if upload_window_required:
    reporting['require_upload_window'] = upload_window_required.lower() in {'1', 'true', 'yes', 'on'}
slotting: dict[str, object] = {}
if upload_slot_count:
    slotting['slot_count'] = max(1, int(upload_slot_count))
if upload_slot_jitter_s:
    slotting['jitter_s'] = max(0, int(upload_slot_jitter_s))
if slotting:
    reporting['slotting'] = slotting
remote_write: dict[str, str] = {}
if prom_remote_write_on_cmd:
    remote_write['on_cmd'] = prom_remote_write_on_cmd
if prom_remote_write_off_cmd:
    remote_write['off_cmd'] = prom_remote_write_off_cmd
if remote_write:
    reporting['remote_write'] = remote_write
if reporting:
    reporting['measure_window'] = {
        'from': policy['range']['from'],
        'to': policy['range']['to'],
    }

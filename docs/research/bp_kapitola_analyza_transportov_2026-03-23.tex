...
\definecolor{protoColor}{RGB}{35,88,153}
\definecolor{grpcColor}{RGB}{30,136,94}
\definecolor{restColor}{RGB}{200,113,42}
\definecolor{jsonrpcColor}{RGB}{156,66,66}
...
\subsection{Diagram benchmark pipeline}

Obrazok~\ref{fig:benchmark-pipeline} sumarizuje krokovy tok benchmarku
od pripravy workloadu az po finalne metriky.
...
\begin{tikzpicture}[
  node distance=7mm,
  box/.style={
    rectangle, rounded corners, draw=black!70, fill=gray!8,
    minimum width=3.8cm, minimum height=0.95cm, align=center, font=\small
  },
  arrow/.style={-Latex, thick, black!70}
]
\node[box] (s1) {1) Generovanie\\schema-validnych sprav};
\node[box, below=of s1] (s2) {2) Vypocet payload\\protobuf vs JSON};
\node[box, below=of s2] (s3) {3) Model transportu\\protobuf-only / gRPC / REST / JSON-RPC};
\node[box, below=of s3] (s4) {4) CPU benchmark\\serialize + parse};
\node[box, below=of s4] (s5) {5) Export artefaktov\\JSON + tabulky};
...
\draw[arrow] (s4) -- (s5);
\end{tikzpicture}
\caption{Diagram postupu benchmarku PREPARE+REPORT}
...
\addplot[fill=protoColor!80, draw=protoColor] coordinates {(small,4509) (medium,10909) (large,34910)};
\addplot[fill=grpcColor!80, draw=grpcColor] coordinates {(small,5384) (medium,11784) (large,35785)};
\addplot[fill=restColor!80, draw=restColor] coordinates {(small,9796) (medium,23346) (large,74159)};
\addplot[fill=jsonrpcColor!80, draw=jsonrpcColor] coordinates {(small,10313) (medium,23863) (large,74676)};
\legend{protobuf-only,gRPC protobuf,REST/JSON,JSON-RPC}
\end{axis}
\end{tikzpicture}
\caption{Chart porovnania celkoveho objemu prenesenych dat}
\label{fig:transport-bytes-chart}
\end{figure}
...
\begin{axis}[
  ...
  ymin=0,
  ymax=21,
  ylabel={Overhead gRPC vs protobuf-only [\%]},
  xlabel={Profil workloadu},
  symbolic x coords={small,medium,large},
  xtick=data,
  ymajorgrids=true,
  grid style={dashed,gray!30},
...
\addplot[
  ...
  line width=1.2pt,
  mark=*,
  mark options={fill=grpcColor}
] coordinates {(small,19.41) (medium,8.02) (large,2.51)};
\end{axis}
\end{tikzpicture}
\caption{Chart poklesu percentualneho overheadu gRPC pri rastu payloadu}
\label{fig:grpc-overhead-profile-chart}
\end{figure}
...
\begin{axis}[
  ...
  ylabel={Rozdiel gRPC vs protobuf-only [\%]},
  ymin=7.5,
  ymax=18,
  ymajorgrids=true,
  grid style={dashed,gray!30},
  xtick={1,5,10,50,100,1000},
  xticklabels={1,5,10,50,100,1000}
]
\addplot[
  color=grpcColor,
  line width=1.2pt,
...
\end{axis}
\end{tikzpicture}
\caption{Chart amortizacie transportneho overheadu pri opakovani sekvencie (medium)}
\label{fig:grpc-overhead-runs-chart}
\end{figure}
...
\addplot[fill=protoColor!80, draw=protoColor] coordinates {(Serialize,0.1438) (Parse,0.1355)};
\addplot[fill=restColor!80, draw=restColor] coordinates {(Serialize,1.1904) (Parse,0.7879)};
\addplot[fill=jsonrpcColor!80, draw=jsonrpcColor] coordinates {(Serialize,1.1827) (Parse,0.8300)};
\legend{protobuf,JSON,JSON-RPC}
\end{axis}
\end{tikzpicture}
\caption{Chart CPU casov serializacie a parsovania}
\label{fig:cpu-serialize-parse-chart}
\end{figure}

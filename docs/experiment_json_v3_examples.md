...
- Metrics and telemetry from `WIFI_SCAN`, `BLE_SCAN`, `CAPTURE_CSI`, command status, and device health should go through Fleet metrics ingest or report maps, then Prometheus, Mimir, and Grafana.
- Grafana reads the `Mimir` datasource. Prometheus is the scrape/remote-write component; Mimir is the durable time-series store.
...
- eLabFTW uploads should stay coarse-grained: measurement evidence/log artifacts, one merged text/log bundle named `wireless-run-evidence-bundle.tar.gz`, and one `run-summary.json` that points readers to Mimir/Grafana instead of duplicating telemetry values.

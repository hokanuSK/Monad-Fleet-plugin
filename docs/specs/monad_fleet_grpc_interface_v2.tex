...
\begin{enumerate}
  ...
  \texttt{Hello} $\rightarrow$ \texttt{GetAssignment} $\rightarrow$ \texttt{GetPolicy} $\rightarrow$ \texttt{AckPrepared}.
...
\texttt{GetAssignment} &
\texttt{agent\_id} &
\texttt{ASSIGNED} or \texttt{NO\_WORK} &
If \texttt{NO\_WORK}, sleep using \texttt{recommended\_prepare\_poll\_sec}. \\
\texttt{GetPolicy} &
\texttt{agent\_id} &
\texttt{OK}, \texttt{NOT\_MODIFIED}, or \texttt{NO\_POLICY} &
...
Treat \texttt{REJECTED} as a control-plane failure; reacquire assignment/policy before measure. \\
\texttt{PublishReport} &
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{process-a.svg}
...
\subsection{Process B: Report Publishing and Event Management}

The following diagram shows the report publishing flow and how events are managed within the system:

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{process-b.svg}
\caption{Process B - Report publishing flow showing device execution, report submission, and event management. Devices execute policies, publish reports, and the system manages event creation and updates.}
\label{fig:process-b}
\end{figure}
...
\begin{itemize}
  \item \texttt{AckPrepared.REJECTED}: reacquire assignment/policy and do not execute stale policy.
...
\begin{itemize}
  \item Control-plane RPCs (\texttt{Hello/GetAssignment/GetPolicy/AckPrepared}): start with 1-2s base delay and cap at 30-60s with jitter.
...
During \texttt{GetAssignment} and later prepare/report RPCs, the server reuses that resource and patches runtime status metadata (\texttt{fleet\_v2\_runtime}) for operational visibility.
...
\begin{enumerate}
  ...
  \item \textbf{No-work polling}: verify \texttt{GetAssignment.NO\_WORK} respects poll interval backoff.
